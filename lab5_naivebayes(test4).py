...
if __name__ == '__main__':
    ...
    print("\n--- Model Result ---")
     # --- Load Your Dataset ---
    own_df = pd.read_csv('/teamspace/studios/this_studio/dataset/hb_test4.csv') # Assuming your CSV is in the same directory

    # --- Separate Features (X) and Target (y) ---
    X_own = own_df.drop([
        'is_canceled', 'reservation_status_date',
        'reservation_status_Canceled', 'reservation_status_No-Show', 'reservation_status_Check-Out'
        ], axis=1) # Features: all columns except 'is_canceled'
    ...
    own_classifier = NaiveBayesClassifier(test_size=0.25, random_state=42, model=BernoulliNB(), stratify=True) # Configurable parameters
