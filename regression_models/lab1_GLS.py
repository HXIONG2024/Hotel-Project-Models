...
class HotelRegressionGLS: # Changed class name to HotelRegressionGLS to indicate GLS
    """
    Performs Generalized Least Squares (GLS) regression on hotel booking data and runs diagnostics.
    """
    def __init__(self, data_path, dependent_variable, features_to_drop):
        """
        Initializes the HotelRegressionGLS class.
    ...
    def fit_model(self):
        """
        Fits the Multiple Linear Regression model using Generalized Least Squares (GLS). # Changed from OLS to GLS in comment
        """
        self.model = sm.GLS(self.y, self.X).fit() # Changed sm.OLS to sm.GLS to use GLS model
    ...
    def print_summary(self):
        ...
        """
        print("\nModel Summary (GLS):") # Added (GLS) to summary title to indicate GLS model
    ...
    def plot_residuals_vs_fitted(self):
        ...
        plt.title('Residuals vs Fitted Values (GLS)') # Added (GLS) to plot title
        plt.savefig('output/residuals_vs_fitted_gls.png') # Changed output file name to indicate GLS
    ...
    def durbin_watson_test(self):
        ...
        print(f'\nDurbin-Watson statistic (GLS): {dw}') # Added (GLS) to output
    ...
    def breusch_pagan_test(self):
        ...
        print('\nBreusch-Pagan test results (GLS):') # Added (GLS) to output
    ...
    def normality_tests(self):
        ...
        plt.title('Q-Q Plot of Residuals (GLS)') # Added (GLS) to plot title
        plt.savefig('output/qq_plot_residuals_gls.png') # Changed output file name to indicate GLS
        ...
        print(f'\nShapiro-Wilk test statistic (GLS): {shapiro_test.statistic}, p-value: {shapiro_test.pvalue}') # Added (GLS) to output
    ...
    def calculate_vif(self):
        ...
        print('\nVariance Inflation Factor (VIF) for each feature (GLS):') # Added (GLS) to output
...
# Instantiate the HotelRegressionGLS class with configuration # Changed to HotelRegressionGLS
hotel_regression_gls = HotelRegressionGLS(DATA_PATH, DEPENDENT_VARIABLE, FEATURES_TO_DROP) # Changed to HotelRegressionGLS

# Prepare data for regression
hotel_regression_gls.prepare_data() # Changed to hotel_regression_gls

# Fit the linear regression model
hotel_regression_gls.fit_model() # Changed to hotel_regression_gls

# Print model summary
hotel_regression_gls.print_summary() # Changed to hotel_regression_gls

# Perform and save diagnostic plots and tests
hotel_regression_gls.plot_residuals_vs_fitted() # Changed to hotel_regression_gls
hotel_regression_gls.durbin_watson_test() # Changed to hotel_regression_gls
hotel_regression_gls.breusch_pagan_test() # Changed to hotel_regression_gls
hotel_regression_gls.normality_tests() # Changed to hotel_regression_gls

# Configure pandas to display all rows and columns
pd.set_option('display.max_rows', None)
pd.set_option('display.max_columns', None)
hotel_regression_gls.calculate_vif() # Changed to hotel_regression_gls
