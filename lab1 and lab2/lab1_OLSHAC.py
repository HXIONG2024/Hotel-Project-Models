...
class HotelRegression:
    ...
    using OLS with HAC robust standard errors.
    """
    def __init__(self, data_path, dependent_variable, features_to_drop):
        ...
        self.robust_results = None # Initialize robust results
    ...
    def fit_model(self):
        ...
        and calculates HAC robust standard errors.
        """
        self.model = sm.OLS(self.y, self.X).fit() # Fit OLS model
        # Calculate HAC robust covariance results
        self.robust_results = self.model.get_robustcov_results(cov_type='HAC', maxlags=37) # You can adjust maxlags
        self.results = self.robust_results.summary() # Get robust model summary

    def print_summary(self):
        """
        Prints the summary of the regression model with robust standard errors.
        """
        print("\n--- Original OLS Model Summary ---")
        print(self.model.summary()) # Print original model summary
        print("\n--- HAC Robust Model Summary ---")
        print(self.results) # Print robust model summary
    ...
    def plot_residuals_vs_fitted(self):
        ...
        residuals = self.model.resid # Get residuals, use .resid from the fitted OLS model
...
# Fit the linear regression model with HAC robust standard errors
hotel_regression.fit_model()

# Print model summary with robust standard errors
hotel_regression.print_summary()

# Perform and save diagnostic plots and tests (these are still based on the OLS model itself)
